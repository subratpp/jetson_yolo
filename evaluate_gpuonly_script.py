...
for num_cpu_cores in range(1, 5, 1):   
    ...
    os.popen("tegrastats --interval 100 --logfile gpustat.tmp &") #start monitoring tegras workload
...
filename = 'gonly_workloadgpu_time.csv'
if not os.path.exists(filename):
    print('creating new file')
    csvfile = open(filename, 'a')
    csvwriter = csv.writer(csvfile)
...
for matrix in range(100, 800, 100):   
    # Assing core to the process
    num_cpu_cores = 1 # number of CPU cores used
    core_list = '' # generate list of cores: string
    for cores in range(num_cpu_cores):
        core_list += "{}".format(cores) #run on core 11
        if cores < num_cpu_cores-1:
            core_list += ','
...
filename = 'gonly_workloadcpu_time.csv'
if not os.path.exists(filename):
    print('creating new file')
    csvfile = open(filename, 'a')
    csvwriter = csv.writer(csvfile)
...
for ram in range(ram_low, ram_high, 500):   
    # Assing core to the process
    num_cpu_cores = 1 # number of CPU cores used
    core_list = '' # generate list of cores: string
    for cores in range(num_cpu_cores):
